...
#Data input
with open('process_MedQuAD_v1.0_train.json') as f:
    raw_train_data = json.load(f)

with open('process_valid.json') as f:
...
####################
##잘 되었는지 확인
pred_start, pred_end = korquad_model.predict(x_eval)
count = 0
eval_examples_no_skip = [_ for _ in eval_squad_examples if _.skip == False]
for idx, (start, end) in enumerate(zip(pred_start, pred_end)):
    squad_eg = eval_examples_no_skip[idx]
    ...
    normalized_pred_ans = normalized_answer(pred_ans)
    normalized_true_ans = normalized_answer(squad_eg.answer_text)
    print(idx,'pred:',normalized_pred_ans)
    print(idx,'True:',normalized_true_ans)
    if normalized_pred_ans in normalized_true_ans:
        count += 1
